...
export default function USACheckoutPage() {
  ...
  const handlePay = async (plan: "consultation" | "quiz") => {
    ...
    try {
      const res  = await fetch("/api/checkout", {
        ...
        body: JSON.stringify({ plan, product: "tsdp" }),
      });
  ...
  const handleCode = async () => {
    ...
    if (data.valid) {
      window.location.href = "/true-self-discovery-profile/quiz?access=quiz&via=code";
    } else {
    ...
    <div style={{
      ...
      <div style={{ textAlign: "center", maxWidth: 680, width: "100%" }}>
        ...
        <p style={{ fontSize: 16, color: "#3a4a3a", marginBottom: 36, lineHeight: 1.6 }}>
          Select the option that fits you best to begin your Unique Self Assessment.
        </p>
